...
NB: I used a recreated version of the PetaLinux repository for my working version in March, so not all of these steps are those I took to actually achieve the version I did then. I did, however, produce working builds that successfully ran cnmr-driver using the code I committed to the GitHub repository.
...
4. CD to `signal-processing-instrument-for-NMR/system_development/Shannon/v0.1/eclypse/software/`.
...
I changed the baud rate to 921600

1. Connect to the board via picocom
```bash
picocom /dev/ttyUSB1 -b 921600 # or your baud rate
```
...
```bash
stty -F /dev/ttyUSB1 921600 # or your baud rate
